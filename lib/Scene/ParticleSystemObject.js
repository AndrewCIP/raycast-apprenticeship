...
export default class ParticleSystemObject extends SceneObject {
  ...
  // GO BACK AND CHECK IF STEPS ARE DONE:
  // -> TODO 2
  ...
  async createParticleGeometry() {
    // Create particles
    this._particles = new Float32Array(this._numParticles * 8); // [x, y, ix, iy, vx, vy, lifespan, initialLifespan]
  ...
  resetParticles() {
    for (let i = 0; i < this._numParticles; ++i) {
      ...
      this._particles[8 * i + 2] = this._particles[8 * i + 0];
      this._particles[8 * i + 3] = this._particles[8 * i + 1];
      // TODO 6: update the velocity
      this._particles[8 * i + 4] = (Math.random() * 2 - 1) * 0.002;
      this._particles[8 * i + 5] = (Math.random() * 2 - 1) * 0.002;
      // Store Random Lifespan
      let life = Math.floor(128 + Math.random() * 127);
      this._particles[8 * i + 6] = life;
      this._particles[8 * i + 7] = life;
    }
    // Copy from CPU to GPU
    this._step = 0;
