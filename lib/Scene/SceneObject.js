 export default class SceneObject {
   ...
   createBindGroup(pipeline) {
     this._bindGroup = this._device.createBindGroup({
       ...
       entries: [{
         binding: 0,
         resource: { buffer: this._transformBuffer }
       }]
     });
   }
  ...
  createTransformBuffer() {
     this._transformBuffer = this._device.createBuffer({
  ...
  updateTransform() {
     const data = new Float32Array([this.position.x, this.position.y]);
     this._device.queue.writeBuffer(this._transformBuffer, 0, data);
  }
