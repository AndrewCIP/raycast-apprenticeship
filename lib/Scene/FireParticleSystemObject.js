...
export default class FireParticleSystemObject extends SceneObject {
  ...
  async createParticleGeometry() {
    // Create particles
    // (this._numParticles * 6) for particles
    // (this._numParticles * 8) for particles
    // (this._numParticles * 12) for gravity_particles
    this._particles = new Float32Array(this._numParticles * 6); // [x, y, ix, iy, vx, vy, lifespan, initialLifespan]
    ...
    // NEW: create time buffer
    this._timeArray = new Float32Array(1);
    this._timeBuffer = this._device.createBuffer({
        label: "Time Buffer " + this.getName(),
        size: this._timeArray.byteLength,
  ...
  resetParticles() {
    ...
    for (let i = 0; i < this._numParticles; ++i) {
      ...
      // spawn at base
        this._particles[idx + 0] = baseX;
        this._particles[idx + 1] = baseY;

        // upward velocity with slight left/right randomness
        const vx = (Math.random() * 2 - 1) * 0.001;   // small horizontal flicker
        const vy = 0.01 + Math.random() * 0.01;       // always upward, fast enough

        this._particles[idx + 2] = vx;
        this._particles[idx + 3] = vy;

        const life = 128 + Math.random() * 127;
        this._particles[idx + 4] = life;

        this._particles[idx + 5] = 1.0; // base size (tune later)
  ...
  compute(pass) { 
    // NEW: update time buffer
    this._timeArray[0] = performance.now() / 1000.0;
    this._device.queue.writeBuffer(this._timeBuffer, 0, this._timeArray);
