...
export default class PGA2D {
  ...
  static windingNumber(polygon, p) {
    let wn = 0;

    // Get polygon normalization info
    const center = polygon._center;        // center of polygon
    const scale = polygon._scaleFactor;    // scale factor used in normalizePolygon

    // Transform mouse point into polygon's local normalized space
    let px = (p[0] - center[0]) / scale;
    let py = (p[1] - center[1]) / scale;
    let pt = [px, py];

    const poly = polygon._polygon; // polygon vertices
