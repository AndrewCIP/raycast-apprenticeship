...
const SHADOW_HARD  = 0;
const SHADOW_SOFT  = 1;
const SHADOW_NAMES = ['Hard Shadow', 'Soft Shadow'];
...
// ── RayBoxShadowObject ────────────────────────────────────────────────────────
// Extends RayBoxLightObject with texture bindings (4-7, same as Scroll 14)
// plus a shadow-flags uniform at binding 8.
class RayBoxShadowObject extends RayBoxLightObject {
  ...
  async createGeometry() {
    ...
    // Stone tile diffuse texture (TGA)
    ...
    // Shadow flags uniform buffer (u32 x 4 = 16 bytes: enabled, mode, pad, pad)
    this._shadowFlagsBuffer = this._device.createBuffer({
      label: 'ShadowFlags Buffer',
      size:  16,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    this._updateShadowFlags();
  }
  ...
  setShadowEnabled(v) { this._shadowEnabled = v; this._updateShadowFlags(); }
  setShadowMode(v)    { this._shadowMode    = v; this._updateShadowFlags(); }

  async createShaders() {
    await super.createShaders();

    this._bindGroupLayout = this._device.createBindGroupLayout({
      label: 'Scroll15 Bind Group Layout',
      entries: [
        ...
        // Texture bindings (carried over from Scroll 14)
        ...
        // Scroll-15 shadow flags
        { binding: 8, visibility: GPUShaderStage.COMPUTE, buffer: {} },
      ],
    });

    this._pipelineLayout = this._device.createPipelineLayout({
      label:            'Scroll15 Pipeline Layout',
  ...
  createBindGroup(outTexture) {
    this._bindGroup = this._device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 8, resource: { buffer: this._shadowFlagsBuffer } },
      ],
    });
...
async function init() {
  ...
  const lightBox = new RayBoxShadowObject(
    renderer._device,
    renderer._canvasFormat,
    camera,
    '/lib/Shaders/scroll_15_light.wgsl'
  );
  ...
  titleEl.textContent = 'Scroll 15 — Shadows';
  ...
  const hudShadowEl = document.createElement('span');
  hudShadowEl.className = 'hud-value';
  addRow(shadowSec, 'Toggle Shadows  (O / o)', ['O', 'o'], hudShadowEl);
  const hudShadowModeEl = document.createElement('span');
  hudShadowModeEl.className = 'hud-value';
  addRow(shadowSec, 'Cycle Mode  (K / k)', ['K', 'k'], hudShadowModeEl);
  ...
  function updateHudShadow() {
    hudShadowEl.textContent     = lightBox._shadowEnabled ? 'ON' : 'OFF';
    hudShadowModeEl.textContent = SHADOW_NAMES[lightBox._shadowMode];
  }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      ...
      // Shadow toggles
      case 'o': case 'O':
        lightBox.setShadowEnabled(!lightBox._shadowEnabled);
        updateHudShadow(); break;
      case 'k': case 'K':
        lightBox.setShadowMode((lightBox._shadowMode + 1) % SHADOW_NAMES.length);
        updateHudShadow(); break;
      // HUD toggle
      case 'h': case 'H':
