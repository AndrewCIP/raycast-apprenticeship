...
// ── Light computation ────────────────────────────────────────────────────────
// Returns raw attenuated intensity and the light-to-surface direction.
// The diffuse dot product is intentionally NOT baked in here so that
// different shading models can use the raw values.
struct LightInfo {
  intensity: vec4f, // attenuated source intensity (no diffuse term yet)
  lightdir:  vec3f, // unit vector pointing FROM light TOWARD surface
}

fn getLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f) -> LightInfo {
  var out: LightInfo;
  let lightType = i32(light.params[2]); // 0=point, 1=directional, 2=spotlight

  if (lightType == 1) {
    // ── Directional light ─────────────────────────────────────────────────
    // Infinite distance → no attenuation, constant direction everywhere.
    out.intensity = light.intensity;
    out.lightdir  = normalize(lightDir);

  } else if (lightType == 2) {
    // ── Spotlight ─────────────────────────────────────────────────────────
    // Cone check: if the hit point falls inside the cutoff cone, apply
    // the spotlight attenuation; otherwise emit nothing.
    let toSurface = normalize(hitPoint - lightPos);
    let cosAngle  = dot(normalize(lightDir), toSurface);
    let cutoff    = light.params[0]; // half-angle in radians
    let dropoff   = light.params[1]; // cone edge fall-off exponent
    ...
    if (cosAngle > cos(cutoff)) {
      ...
      out.intensity = vec4f(0., 0., 0., 0.); // outside cone — no contribution
    }
    out.lightdir = toSurface;

  } else {
    // ── Point light (default) ──────────────────────────────────────────────
    // Radiates uniformly in all directions; falls off with distance squared.
    let toSurface = normalize(hitPoint - lightPos);
    let dist      = length(hitPoint - lightPos);
    let atten     = light.attenuation[0]
                  + dist * light.attenuation[1]
...
// Maps a continuous value in [0,1] to one of five discrete bands,
// mimicking hand-drawn cel shading.
...
// ── Shading dispatch ─────────────────────────────────────────────────────────
// Combines the surface material, light info, and camera direction into a
// final colour using the shading model stored in light.params[3].
fn shadeSurface(
  emit:        vec4f,  // emitted colour (usually zero)
  diffuseCol:  vec4f,  // kd — surface diffuse colour
  normal:      vec3f,  // world-space outward surface normal
  lightInfo:   LightInfo,
  camPos:      vec3f,  // world-space camera position
  hitPt:       vec3f   // world-space hit point
) -> vec4f {
  let l = lightInfo.lightdir;  // FROM light TO surface (negate for Lambertian)
  let I = lightInfo.intensity;
  let shadingModel = i32(light.params[3]); // 0=Lambertian, 1=Phong, 2=Toon

  // Diffuse term — shared by all models
  let diffuseTerm = max(dot(normal, -l), 0.0);

  if (shadingModel == 1) {
    // ── Phong shading ──────────────────────────────────────────────────────
    // Lo = ke + kd*I*(n·-l) + ks*I*(v·-R)^gamma + ka*I
    let R            = reflect(l, normal);              // reflected light direction
    let viewDir      = normalize(camPos - hitPt);       // from surface to camera
    let specularTerm = pow(max(dot(viewDir, -R), 0.0), 64.0);
    let ks           = vec4f(0.5, 0.5, 0.5, 0.0);      // specular material (white shine)
    ...
    // ── Toon shading ───────────────────────────────────────────────────────
    // Same as Phong but diffuse and specular terms are quantized into bands.
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (hitInfo.x > 0) {
    ...
    let diffuseCol = boxDiffuseColor(i32(hitInfo.y));
    var normal    = boxNormal(i32(hitInfo.y));
    normal        = transformNormal(normal);

    // Transform light into world coordinates (camera-stationary trick)
    ...
    // Camera position in world space (origin of camera frame)
    let camPos = applyMotorToPoint(vec3f(0, 0, 0), cameraPose.motor);

    color = shadeSurface(emit, diffuseCol, normal, lightInfo, camPos, hitPt);
  }
